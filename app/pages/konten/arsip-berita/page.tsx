// app/konten/arsip-berita/page.tsx
'use client';
import React, { useMemo, useState } from 'react';
import Link from 'next/link';
import PageLayout from '@/app/component/pagelayout';
import { newsData } from '@/app/data/newsData'; // pastikan newsData items punya id and/or slug
...
export default function ArsipBeritaPage(): React.ReactElement {
  ...
  // helper buat link internal aman (pakai slug / id jika ada)
  const makeNewsLink = (item: any) =>
    item && (item.slug || item.id)
      ? `/pages/konten/arsip-berita/${item.slug || item.id}`
      : '/pages/konten/arsip-berita';
        ...
        <div style={twoCol}>
          ...
          <div style={leftCol}>
            <section style={gridStyle}>
              {visible.map((item, idx) => {
                const linkHref = makeNewsLink(item);
                // gunakan Link tanpa nested <a> (beri style langsung ke Link)
                return (
                  <Link
          ...
          <aside style={rightCol}>
            ...
            <div style={{ background: '#fff', borderRadius: 12, padding: 14, boxShadow: '0 6px 18px rgba(0,0,0,0.04)', marginBottom: 18 }}>
              ...
              {latest.map((it, i) => {
                const linkHref = makeNewsLink(it);
                return (
                  <Link key={(it as any).id ?? (it as any).slug ?? i} href={linkHref} style={sideItemStyle}>
                    <div style={{ display: 'flex', gap: 10, textDecoration: 'none', color: 'inherit' }}>
            ...
            <div style={{ background: '#fff', borderRadius: 12, padding: 14, boxShadow: '0 6px 18px rgba(0,0,0,0.04)' }}>
              ...
              {popular.map((it, i) => {
                ...
                return (
                  <Link key={(it as any).id ?? (it as any).slug ?? i} href={linkHref} style={sideItemStyle}>
                    <div style={{ display: 'flex', gap: 10, alignItems: 'center', textDecoration: 'none', color: 'inherit' }}>
